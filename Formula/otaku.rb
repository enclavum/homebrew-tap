class Otaku < Formula
  ...
  depends_on "rust" => :build   # builds the cryptography extension from source
  depends_on "openssl@3"        # cryptography links against it
  depends_on "python@3.13"

  # Runtime dependency closure — regenerate with `brew update-python-resources otaku`.
  ...
  def install
    # cryptography's Rust build links against Homebrew's OpenSSL.
    ENV["OPENSSL_DIR"] = formula_opt_prefix("openssl@3")
    virtualenv_install_with_resources
  end

  test do
    assert_match "otaku #{version}", shell_output("#{bin}/otaku --version")
  end
end
